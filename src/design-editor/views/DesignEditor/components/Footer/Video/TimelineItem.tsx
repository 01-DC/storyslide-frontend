...
const TimelineItem = ({
	...
	const onMouseMoveItem = (evt: any) => {
		if (timeLineItemRef.current) {
			const position =
				evt.pageX -
				timeLineItemRef.current?.getBoundingClientRect().left
	...
	const setTimeByMarker = React.useCallback(
		(id: string, change: number) => {
			...
			if (currentIndex > -1) {
				...
				const prevDuration = prevScenes.reduce(function (
					previousVal,
					currentValue
				) {
	...
	React.useEffect(() => {
		...
		return () => {
			if (timeLineItemDiv) {
				timeLineItemDiv.removeEventListener(
					"contextmenu",
					handleContextMenu
				)
			}
