...
>("uploads/getUploads", async (_, { rejectWithValue, dispatch }) => {
	try {
...
export const uploadFile = createAsyncThunk<void, { file: File }, any>(
	...
	async (args, { dispatch }) => {
		...
		const response = await api.getSignedURLForUpload({
			name: updatedFileName,
		})
		await axios.put(response.url, updatedFile, {
			headers: {
				"Content-Type": mime.getType(updatedFileName) as string,
			},
		...
		const uploadedFile = await api.updateUploadFile({
			name: updatedFileName,
		})
