...
class ImageMapEditor extends Component {
	...
	canvasHandlers = {
		...
		onSelect: (target) => {
			...
			if (
				...
				this.canvasRef.handler.getObjects().forEach((obj) => {
					if (obj) {
						this.canvasRef.handler.animationHandler.resetAnimation(
							obj,
							true
						)
					}
		...
		onChange: (selectedItem, changedValues, allValues) => {
			...
			if (
				...
				if (selectedItem.type === "image") {
					this.canvasRef.handler.setImageById(
						selectedItem.id,
						changedValue
					)
			...
			if (changedKey === "icon") {
				const { unicode, styles } =
					changedValue[Object.keys(changedValue)[0]]
		...
		onChangeWokarea: (changedKey, changedValue, allValues) => {
			...
			if (changedKey === "width" || changedKey === "height") {
				...
				this.canvasRef.canvas.centerObject(
					this.canvasRef.handler.workarea
				)
				return
			}
	...
	handlers = {
		...
		onImport: (files) => {
			if (files) {
				...
				setTimeout(() => {
					...
					reader.onload = (e) => {
						const { objects, animations, styles, dataSources } =
		...
		onDownload: () => {
			...
			anchorEl.download = `${
				this.canvasRef.handler.workarea.name || "sample"
			}.json`
