...
import { getStoryData, putStoryData } from "../adapters/story"
...
function Title({ id }) {
  ...
  const getStoryTitle = async () => {
    try {
      ...
      if (data.title !== "") setInputValue(data.title)
      else {
        let d = new FormData()
        d.append("title", "My Project " + id)
        setInputValue("My Project " + id)
        await putStoryData(d, id)
      }
    } catch (error) {
      console.log(error)
    }
  ...
  return (
    <div style={{ fontWeight: "normal" }}>
      {vis ? (
        ...
        <u onClick={() => setVis(true)} className="  mt-0">
          {inputValue}
        </u>
      )}
    </div>
